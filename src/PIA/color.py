...
def loadtxt(name: str) -> NDArray[int]:
    r = []
    with open(name) as f:
        for ln in f:
            h = ln.lstrip("#")
...
def tolab(rgb: NDArray[float], normalized: bool = False) -> NDArray[float]:
    if not normalized:
        rgb = np.divide(rgb, 255.0)
    rgb = rgb.reshape(-1, 3)

    c = rgb > 0.04045
    rgb[c] = np.power((rgb[c] + 0.055) / 1.055, 2.4)
    rgb[~c] = rgb[~c] / 12.92
    xyz = np.matmul(rgb, tolab.A) / tolab.b

    c = xyz > 0.008856
    xyz[c] = np.power(xyz[c], 1.0 / 3.0)
    xyz[~c] = xyz[~c] * 7.787 + 16.0 / 116.0
    return np.stack(
        [
            (116 * xyz[:, 1]) - 16,
            500 * (xyz[:, 0] - xyz[:, 1]),
...
def quantize(
    im: NDArray[np.uint8], colors: NDArray[np.uint8], threshold: float = 0.12
) -> Tuple[NDArray[np.uint8], NDArray[float]]:
    X = tolab(im)
    c = tolab(colors)
    centers, labels = cluster.kmeans(X, c, th=threshold * 100.0)
    IM = labels.reshape(*im.shape[:2]).astype(np.uint8)
    return IM, centers
...
def fill(
    ...
    m = im > 0
    x = np.stack(np.nonzero(m), axis=1)
    y = im[m]
...
tolab.b = np.array([0.95047, 1.00000, 1.08883])
...
class Palette:
    def __init__(self, name, num):
        colors = brewer[name][num]
        self._p = np.array(colors) / 255.0
        self.set()
    ...
    def __next__(self):
        self._i = (self._i + 1) % self._p.shape[0]
